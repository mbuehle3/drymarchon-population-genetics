...
# setwd("/home/mbuehle3/Dropbox/dissertation/lab-notebooks/drymarchon/dpg-data/run-04/fineRADstructure/all-asm-08/data/run-04/") ## The directory where the files are located

chunkfile<-"data/run-04/populations-09_chunks.out" ## RADpainter output file
mcmcfile<-"data/run-04/populations-09_mcmc-chunks.out" ## finestructure mcmc file
treefile<-"data/run-04/populations-09_mcmctree-chunks.out" ## finestructure tree file
### 2) EDIT THIS PATH TO WHERE YOU WANT THE PLOTS:
plotsFolder <- "./run-06-plots/"
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- "run-06";  maxIndv <- 116; maxPop<-300



### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source("/home/mbuehle3/programs/fineRADstructure/FinestructureLibrary.R", chdir = TRUE) # read in the R functions, which also calls the needed packages
...
###### READ IgN THE CHUNKCOUNT FILE
dataraw<-as.matrix(read.table(chunkfile,row.names=1, header = TRUE)) # read in the pairwise coincidence 
...
dim(dataraw)
setdiff(fullorder, colnames(dataraw))

fullorder<-labels(tdend)

fullorder
colorder <- gsub("-", "_",fullorder)
colorder <- gsub("\\.", "_",fullorder)
colorder


print(row.names(dataraw))
...
pdf(file=paste(plotsFolder,analysisName,"-SimpleCoancestry.pdf",sep=""),height=20,width=20)
par(mar=c(4,4,4,4))

plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.4,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1))
...
dev.off()
    ...
    pdf(file=paste(plotsFolder,analysisName,"-PopAveragedCoancestry.pdf",sep=""),height=45,width=45)
    par(mar=c(4,4,4,4))
    ...
    pdf(file=paste(plotsFolder,analysisName,"-PopAveragedCoancestry2.pdf",sep=""),height=20,width=20)
    par(mar=c(4,4,4,4))
